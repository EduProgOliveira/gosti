...
class AppPreferences {
  ...
  static token(String token) async => await SharedPreferences.getInstance()
      .then((prefs) => prefs.setString('token', token));
  static id(int id) async => await SharedPreferences.getInstance()
      .then((prefs) => prefs.setInt('id', id));
  static email(String email) async => await SharedPreferences.getInstance()
      .then((prefs) => prefs.setString('email', email));
  static password(String password) async =>
      await SharedPreferences.getInstance()
          .then((prefs) => prefs.setString('password', password));
  static phone(String phone) async => await SharedPreferences.getInstance()
      .then((prefs) => prefs.setString('phone', phone));
  static keepConnected(bool keepConnected) async =>
      await SharedPreferences.getInstance()
          .then((prefs) => prefs.setBool('keepConnected', keepConnected));
  static validade(String validade) async =>
      await SharedPreferences.getInstance()
          .then((prefs) => prefs.setString('validade', validade));
  static mensagem(String mensagem) async =>
      await SharedPreferences.getInstance()
          .then((prefs) => prefs.setString('mensagem', mensagem));
    ...
    if (!await VALDADE_CHECK_TOKEN()) {
      if (keepConnected) {
        if (token.isNotEmpty) {
          ...
          return await AuthenticationRepository.reLogin(
