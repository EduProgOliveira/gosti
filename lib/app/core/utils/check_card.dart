class CheckCard {
  static Map? card(String number) {
    ...
    if (elo.hasMatch(number)) {
      return {'bin': 'elo', 'lengthcart': '12'};
    }
    if (mastercard.hasMatch(number)) {
      return {'bin': 'master', 'lengthcart': '16'};
    }

    if (hipercard.hasMatch(number)) {
      return {'bin': 'hipercard', 'lengthcart': '16'};
    }
    if (aura.hasMatch(number)) {
      return {'bin': 'aura', 'lengthcart': '16'};
    }
    if (visa.hasMatch(number)) {
      return {'bin': 'visa', 'lengthcart': '16'};
    }
    if (diners.hasMatch(number)) {
      return {'bin': 'diners', 'lengthcart': '16'};
    }
    if (amex.hasMatch(number)) {
      return {'bin': 'amex', 'lengthcart': '16'};
    }
    if (discover.hasMatch(number)) {
      return {'bin': 'discover', 'lengthcart': '16'};
    }
    if (jbc.hasMatch(number)) {
      return {'bin': 'jbc', 'lengthcart': '16'};
    }
    return {'bin': ''};
  }
}
